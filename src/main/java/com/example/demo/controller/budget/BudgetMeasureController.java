...
public class BudgetMeasureController extends BaseController<BudgetMeasure> {
  ...
  public ResData getTree2(HttpServletRequest request) {
    ...
    if (err == null) {
      ...
      userPage = budgetMeasureService.page(page, parseWrapperOption(wrapperOpt2));
      List<BudgetMeasure> list2 = userPage.getRecords();
      // 放入大数组
      ...
      for (int i = 0; i < prelist.size(); i++) {
        ...
        if (value.fetchParentId().toString().equals(selectId)) {
          prelist.remove(i);
          relist.add(budgetMeasureService.treeBusinessParse(value, prelist));
          i = -1;
        }
      }
      List<BudgetMeasure> resist = new ArrayList<BudgetMeasure>();
