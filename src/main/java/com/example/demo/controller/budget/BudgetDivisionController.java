...
public class BudgetDivisionController extends BaseController<BudgetDivision> {
  ...
  public ResData getTree2(HttpServletRequest request) {
    ...
    if (err == null) {
      ...
      userPage = budgetDivisionService.page(page, parseWrapperOption(wrapperOpt2));
      List<BudgetDivision> list2 = userPage.getRecords();
      // 放入大数组
      ...
      for (int i = 0; i < prelist.size(); i++) {
        ...
        if (value.fetchParentId().toString().equals(selectId)) {
          prelist.remove(i);
          relist.add(budgetDivisionService.treeBusinessParse(value, prelist));
          i = -1;
        }
      }
