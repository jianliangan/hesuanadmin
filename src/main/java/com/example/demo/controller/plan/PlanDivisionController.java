...
public class PlanDivisionController extends BaseController<PlanDivision> {
  ...
  public ResData getTree2(HttpServletRequest request) {
    ...
    if (err == null) {
      ...
      userPage = planDivisionService.page(page, parseWrapperOption(wrapperOpt2));
      List<PlanDivision> list2 = userPage.getRecords();
      // 放入大数组
      ...
      for (int i = 0; i < prelist.size(); i++) {
        ...
        if (value.fetchParentId().toString().equals(selectId)) {
          prelist.remove(i);
          relist.add(planDivisionService.treeBusinessParse(value, prelist));
          i = -1;
        }
      }
      List<PlanDivision> resist = new ArrayList<PlanDivision>();
