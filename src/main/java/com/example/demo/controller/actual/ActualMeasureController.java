...
public class ActualMeasureController extends BaseController<ActualMeasure> {
  ...
  @Autowired private IActualMeasureService actualMeasureService;
  ...
  private void convertProject2Measure(Project project, List list) {
    ActualMeasure p1 = new ActualMeasure();
  ...
  public ResData getTree2(HttpServletRequest request) {
    ...
    if (err == null) {
      ...
      userPage = actualMeasureService.page(page, parseWrapperOption(wrapperOpt2));
      List<ActualMeasure> list2 = userPage.getRecords();
      // 放入大数组
      ...
      for (int i = 0; i < prelist.size(); i++) {
        ...
        if (value.fetchParentId().toString().equals(selectId)) {
          prelist.remove(i);
          relist.add(actualMeasureService.treeBusinessParse(value, prelist));
          i = -1;
        }
      }
      List<ActualMeasure> resist = new ArrayList<ActualMeasure>();
