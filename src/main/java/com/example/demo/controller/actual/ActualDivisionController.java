...
public class ActualDivisionController extends BaseController<ActualDivision> {
  ...
  private void convertProject2Division(Project project, List list) {
  ...
  public ResData getTree2(HttpServletRequest request) {
    ...
    if (err == null) {
      ...
      for (int i = 0; i < tmplist.size(); i++) {
        ...
        // 放入大数组
        convertProject2Division((Project) tmplist.get(i), prelist);
      }
      ...
      userPage = actualDivisionService.page(page, parseWrapperOption(wrapperOpt2));
      List<ActualDivision> list2 = userPage.getRecords();
      // 放入大数组
      ...
      for (int i = 0; i < prelist.size(); i++) {
        ...
        if (value.fetchParentId().toString().equals(selectId)) {
          prelist.remove(i);
          relist.add(actualDivisionService.treeBusinessParse(value, prelist));
          i = -1;
        }
      }
      List<ActualDivision> resist = new ArrayList<ActualDivision>();
      convertProject2Division(project, resist);
