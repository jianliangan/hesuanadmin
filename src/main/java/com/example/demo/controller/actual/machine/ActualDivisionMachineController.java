...
public class ActualDivisionMachineController extends BaseController<ActualDivisionMachine> {
    ...
    public ResData getTree(HttpServletRequest request) {
        ...
        List<ActualDivisionMachine> relist = null;
        int pageIndex = 1;
        String err = null;
        IPage userPage = null;
        ...
        if (err == null) {
            ...
            // Page<ActualDivisionMachine> page = new Page(pageIndex, 5000);
            QueryWrapper wrapper = fetchWrapper(request);

            // userPage = fetchService().list(page, wrapper);
            ...
            for (int i = 0; i < list.size(); i++) {
                ActualDivisionMachine value = (ActualDivisionMachine) list.get(i);
            ...
            relist = new ArrayList<ActualDivisionMachine>();
            for (int i = 0; i < list.size(); i++) {
                ActualDivisionMachine value = (ActualDivisionMachine) list.get(i);
                if (value.fetchParentId().toString().equals(rootId)) {
                    list.remove(i);
                    relist.add((ActualDivisionMachine) ITreeService.treeLoop0(value, (List<Base>) list));
                    i = -1;
                }
            }
...
/////
    ...
    public void filterUpdateRow(String pre, String cmd, ActualDivisionMachine row, HttpServletRequest request) {
